...
def choice_photo(call: CallbackQuery):
    """
    Функция, которая уточняет у пользователя необходимость вывода фотографий к отелям в формате inline-кнопок.
    :param call: CallbackQuery
    :return: None
    """
    logger.info(str(call.from_user.id))
...
def callback_count_photo(call: CallbackQuery) -> None:
    ...
    Исходя из стартовых данных у экземпляра класса UserHandle, направляет пользователя далее.
    Если аргумент 'count_hotel', имеет стартовое значение (0), то переходим в функцию date_in, файла calendar,
